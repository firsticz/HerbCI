...
<body>
    ...
    <div class="row justify-content-center align-items-center">
        ...
        let score = $("#result");
        let i = 1;
        $.post("http://localhost:8080/Mhunpris/api/usersocre/findagv", {},
            function (data, textStatus, jqXHR) {
                console.log(data);
                var listOfscore = data.data;
                var i=0;
                var strScore = "";
                $.each(listOfscore, function( index, value ) {
                    strScore += '<h4>'+value.drugformulaId+'</h4>'
                                +'<p>คะแนนความพึงพอใจ</p>'
                               + '<div class="progress mb-3">'
                               + '<div class="progress-bar" role="progressbar" style="width: '+data.data2.avg[i]+'%;" aria-valuenow="'+data.data2.avg[i]+'" aria-valuemin="0" aria-valuemax="100">'+data.data2.avg[i]+'%</div>'
                               + '</div>'
                            i++;            
                });
                score.html(strScore);  
